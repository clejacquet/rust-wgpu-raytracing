...
pub async fn load_model_compute(
    file_name: &str,
    device: &wgpu::Device,
    queue: &wgpu::Queue,
    layout: &wgpu::BindGroupLayout,
    ...
    let meshes = models
        ...
        .map(|m| {
            ...
            let faces = (0..m.mesh.indices.len() / 3)
                .map(|i| {
                    model::ModelFaceSmall::new([
                        m.mesh.indices[i * 3],
                        m.mesh.indices[i * 3 + 1],
                        m.mesh.indices[i * 3 + 2],
                    ])
                })
                .collect::<Vec<_>>();
...
pub struct MeshData {
    ...
    pub texcoords: Vec<cgmath::Vector2<f32>>,
...
pub async fn load_model_data_compute(
    file_name: &str,
    device: &wgpu::Device,
    queue: &wgpu::Queue,
    layout: &wgpu::BindGroupLayout,
    ...
    let meshes = models
        ...
        .map(|m| {
            ...
            let texcoords = (0..m.mesh.positions.len() / 3)
                .map(|i| {
                    cgmath::Vector2::<f32>::new(
